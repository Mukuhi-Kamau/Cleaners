...
$wcee = explode(',',strtoupper ($wambua_corrupted_employee));


$w2 = array_splice(array_unique($wcee), 0, 7);
...
foreach ($w2 as $value ) {
  ...
  $nvalue=  preg_replace('/[0-9@#$%>>+=<\/-]+/', '', $value);
  echo "<td><strong>$nvalue</strong></td>";

}
?>
</tr>
<?php
$w4 = explode(",",strtolower($wambua_corrupted_employee));
$w3 = array_splice(array_unique($w4), 8, 254);
$array_of_rows = array_chunk($w3,7);
...
while( $a <= 7){
    ...
    $a++;
//tried to extract_nonumbers

//function extract_nonnumbers ($wcee){
  //preg_match_all('#[^\d]#',$wcee, $match, $cell);
    //return $match [0];
//}
...
echo "<td>{$cell2}</td>";
  ...
  echo '</tr>';
  //Removing all symbols and numbers
